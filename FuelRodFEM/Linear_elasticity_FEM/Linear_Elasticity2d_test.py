...
KK = integrator1.assembly_cell_matrix_fast(space=vspace)
bform.assembly()
K = bform.get_matrix()

integrator2 = VectorMassIntegrator(c=1, q=p+1)
...
integrator3 = VectorSourceIntegrator(f=pde.source, q=p+1)
...
error = mesh.error(u=uh, v=u_exact, q=p+5, power=2)
print('error:',error)
output = './mesh_linear/'
if not os.path.exists(output):
    os.makedirs(output)
fname = os.path.join(output, 'linear_elastic2d.vtu')
