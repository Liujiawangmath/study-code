...
for n in range(nt):
    t = duration[0] + n*tau
    print(F_caldding.shape,F_inner.shape)
    print(M_caldding.shape,M_inner.shape)
    print(K_caldding.shape,K_inner.shape)
    import numpy as np

    # 假设您已经初始化了 M_caldding、M_inner、K_caldding、K_inner、alpha_caldding 和 alpha_inner
    # 确保它们的形状都是正确的

    # 假设 F_caldding 和 F_inner 是一维数组，按照指定的顺序连接成一个大的一维数组
    F_combined = np.concatenate((F_caldding, F_inner))

    # 使用 np.block 函数将 M_caldding、M_inner、K_caldding、K_inner、alpha_caldding 和 alpha_inner 组装成一个大矩阵和一个大的常数向量
    M_combined = np.block([[M_caldding, np.zeros((324, 161))],
                       [np.zeros((161, 324)), M_inner]])

    K_combined = np.block([[K_caldding, np.zeros((324, 161))],
                       [np.zeros((161, 324)), K_inner]])

    alpha_combined = np.concatenate((alpha_caldding, alpha_inner))

    # 假设 p 是一个一维数组，根据您的代码，似乎需要将其形状调整为列向量
    p = p.reshape(-1, 1)

    # 根据您的公式计算 A_combined 和 b_combined
    A_combined = M_combined + tau * alpha_combined * K_combined
    b_combined = M_combined @ p + tau * F_combined
